...
api.init(app, server).then(async () => {
  ...
  server.listen(port, () => {
    ...
    console.log(
      chalk.green(`| Workers Spawned: ${Object.keys(cluster.workers).length}          |`)
    );
    console.log(chalk.green(`*-----------------------------*`));
  });
  ...
  if (cluster.isMaster) {
    const clustersQuantity =
      process.env.FORKS === 'MAX' ? os.cpus().length : process.env.FORKS || 0;
    const workers = [];
    ...
    cluster.on('exit', function (worker, code, signal) {
      console.log(
        chalk.red(`Worker ${worker.process.pid} died with code ${code} and signal ${signal}`)
      );
